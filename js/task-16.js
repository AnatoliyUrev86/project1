...
// Для того щоб в кожне віконичко привоїти значення наших промісів потрібно отримати масив із значеннями
// для цього ми використовуємо метод  Promise.allSettled він приймає масив наших промісів (promises)
// ми отримуємо проміс в якого в консолі є властивість value - це коли проміс завершився успішно і властивість
// rejected - це коли проміс завершився помилкою це саме специфіка роботи методу allSettled це він нам формує
// ось таку конструкцію
// Наш масив промісів ми перебираємо за допомогою методу forEach ((item, i)) item - це елемент нашого масиву
// i - індекс за яким ми будемо до наших елементів звертатися
// Для того щоб розтавити наші елементи по місцях відповідно їх індексів ми звертаємось до container та
// використовуємо властивість children та в квадратних дужках пишемо індекс [i] потім використовуємо
// властивість textContent = "" і присвоюємо її пустий рядок щоб при повторному натисненні на кнопку
// очищати наші елементи а потім зповнювати їх заново ми це робимо перед setTimeout щоб з початку
// очистити поля а потім щоб відображалися наші елемнти повторно
// textContent = item.value || item.reason може бути при запуску в консолі item.value або item.reason
// Нам потрібен метод який допоможе відкладено зробити дію  setTimeout тобто наш асинхроний код і в нього
// ми поміщаємо елементи які ми хочемо щоб відобразились
// у консолі container.children[i].textContent = item.value || item.reason
// Тепер потрібно зробити щоб наші елементи відображалися по черзі для цього ми нашу затримку в часі
// можемо помножити на індекс щоб затримку при відображенні мали всі наші елементи одночасно 1000 * (i + 1))
// і до індекса додаємо одиницю щоб відображення починалося не з першого індекса а не з нулевого
// Після того ми маємо виводити повідомлення виграв наш гравець чи програв для цього в нашому  setTimeout
// пишемо  result.textContent = isWinner ? "Winner" : "Luser"
